...
class HAM10000(data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, process=False):
        ...
        if process:
            if(os.path.exists(os.path.join(self.root, self.raw_folder)) and \
               os.path.exists(os.path.join(self.root, self.raw_folder))):
                self.process()
            else:
    ...
    def process(self):
        ...
        data_links = images_and_labels_path(os.path.join(self.root, self.raw_folder))
        #train_img_and_lbl_dirs = get_labels_and_files(train_data_links)
        #test_img_and_lbl_dirs = get_labels_and_files(test_data_links)
        img_and_lbl_dirs = get_labels_and_files(data_links)
        #random.shuffle(train_img_and_lbl_dirs)
        #random.shuffle(test_img_and_lbl_dirs)
        random.shuffle(img_and_lbl_dirs)
        total_imgs = len(img_and_lbl_dirs)
        num_train_imgs = int(0.8*total_imgs)
        train_img_and_lbl_dirs = img_and_lbl_dirs[0:num_train_imgs]
        test_img_and_lbl_dirs = img_and_lbl_dirs[num_train_imgs:]
